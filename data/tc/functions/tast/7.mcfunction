...
execute if score countdown command matches 160 run title @a title {"text": "跳到岩浆里"}
...
execute if score countdown command matches 160.. run title @a actionbar {"text": "§a目前挑战 §f跳到岩浆里"}

#偵測這個玩家執行了條件式，並讓該玩家在該位置執行 function tc:getscore
execute if score countdown command matches 160.. as @a at @s if block ~ ~ ~ lava run function tc:getscore