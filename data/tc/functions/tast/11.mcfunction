...
execute if score countdown command matches 160 run title @a title {"text": "杀一只只因"}
...
execute if score countdown command matches 160.. run title @a actionbar {"text": "§a目前挑战 §f杀一只只因"}

#偵測這個玩家執行了條件式，並讓該玩家在該位置執行 function tc:getscore
execute if score countdown command matches 160.. as @a[scores={kill_chicken=1}] at @s run function tc:getscore