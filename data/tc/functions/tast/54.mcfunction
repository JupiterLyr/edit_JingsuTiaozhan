...
execute if score countdown command matches 160 run title @a title {"text": "贴在一只骷颅弓箭手身上"}
...
execute if score countdown command matches 160.. run title @a actionbar {"text": "§a目前挑战 §f贴在一只骷颅弓箭手身上"}

#偵測這個玩家執行了條件式，並讓該玩家在該位置執行 function tc:getscore

execute if score countdown command matches 160.. as @a at @s if entity @e[type=skeleton,distance=..1] run function tc:getscore