...
execute if score countdown command matches 160 run scoreboard players reset * drop_charcoal

#編寫該挑戰出現時應該要顯示的訊息
execute if score countdown command matches 160 run title @a subtitle {"text": "第一个完成挑战的玩家获得分数","color": "gray"}
execute if score countdown command matches 160 run title @a title {"text": "丢掉一块木炭"}
