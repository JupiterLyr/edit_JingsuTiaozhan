...
execute if score countdown command matches 160 run title @a title {"text": "§f在副手放一个铁块"}
...
execute if score countdown command matches 160.. run title @a actionbar {"text": "§a目前挑战 §f在副手放一个铁块"}

#偵測這個玩家執行了條件式，並讓該玩家在該位置執行 function tc:getscore
execute if score countdown command matches 160.. as @a at @s if entity @s[nbt={Inventory:[{Slot:-106b,id:"minecraft:iron_block"}]}] run function tc:getscore